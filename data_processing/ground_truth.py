...
# --- Phần 1: Ground Truth Creation (Tự động) ---
# Lưu ý: Với LEVIR-CD, bạn đã có nhãn Label (GT) sẵn, nên phần tự động này được bỏ qua.
# Nhưng nếu cần tạo GT tự động từ ảnh SAR (Nhóm MINH):

def create_auto_change_mask(before_img: np.ndarray, after_img: np.ndarray):
    """Tạo Change Mask tự động bằng Image Differencing và Otsu threshold."""
    
    # 1. Image Differencing (Tính độ khác biệt)
    diff = np.abs(before_img.astype(np.float32) - after_img.astype(np.float32))
    
    # 2. Chuyển sang 1 kênh nếu là ảnh RGB
    if diff.ndim > 2:
    ...
    # 4. Morphological closing (Post-processing nhẹ)
    kernel = np.ones((5, 5), np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
    
    return mask.astype(np.float32) / 255.0 # Chuẩn hóa về 0 hoặc 1

# --- Phần 2: Xử lý Nhãn Thô (Nhóm MINH) ---

def process_raw_label(label_img: np.ndarray) -> np.ndarray:
    """Xử lý nhãn thô để đảm bảo nó là nhị phân (0 hoặc 1)."""
    # Nhãn LEVIR-CD thường có giá trị 0 (không đổi) và 255 (thay đổi)
    binary_mask = (label_img > 0).astype(np.float32)
    return binary_mask